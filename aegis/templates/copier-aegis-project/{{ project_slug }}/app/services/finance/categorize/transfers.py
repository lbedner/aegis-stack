...
async def detect_transfers(
    ...
    *,
    ...
    result.payment_paired = await _pair_payment_history(
        db, owner_user_id=owner_user_id
    )
    return result
...
async def _flag_category_transfers(
    ...
    """
    transfer_categories = select(FinanceCategory.id).where(
    ...
    rows = (
        await db.exec(
            select(FinanceTransaction).where(
                ...
                FinanceTransaction.is_transfer.is_(False),
                FinanceTransaction.transfer_group_id.is_(None),
                FinanceTransaction.category_id.in_(transfer_categories),
                _owner_clause(FinanceTransaction.owner_user_id, owner_user_id),
            )
        )
    ).all()
...
_ADJUSTMENT_TOKENS = frozenset({"ADJ", "ADJUSTMENT", "REDIST", "REVERSAL", "CORRECTION"})
...
def _looks_like_adjustment(txn: FinanceTransaction) -> bool:
    ...
    for text in (txn.name, txn.original_description):
        if not text:
            continue
        tokens = {t for t in "".join(c if c.isalnum() else " " for c in text.upper()).split()}
        if tokens & _ADJUSTMENT_TOKENS:
            return True
    return False


async def _flag_adjustment_pairs(
    db: AsyncSession, *, owner_user_id: int | None
) -> int:
    ...
    anchors = (
        await db.exec(select(FinanceTransaction).where(*base_filters, token_clause))
    ).all()
    anchor_keys = {(t.account_id, t.date_) for t in anchors if _looks_like_adjustment(t)}
    if not anchor_keys:
        return 0
...
async def _pair_payment_history(
    db: AsyncSession, *, owner_user_id: int | None
) -> int:
    ...
    def _unpaired(account_ids: set[int], inflow: bool):
        amount_clause = (
            FinanceTransaction.amount > 0 if inflow else FinanceTransaction.amount < 0
        )
        return select(FinanceTransaction).where(
            FinanceTransaction.deleted_at.is_(None),
            FinanceTransaction.dedup_status != "duplicate",
            FinanceTransaction.transfer_group_id.is_(None),
            amount_clause,
    ...
    inflows = (await db.exec(_unpaired(liability_ids, inflow=True))).all()
    if not inflows:
        return 0
    outflows = (await db.exec(_unpaired(asset_ids, inflow=False))).all()
    ...
    for in_txn in sorted(inflows, key=lambda t: t.date_):
        candidates = [
            ...
            if t.id not in used
            and abs((t.date_ - in_txn.date_).days) <= WINDOW_DAYS
        ]
        if not candidates:
            continue
