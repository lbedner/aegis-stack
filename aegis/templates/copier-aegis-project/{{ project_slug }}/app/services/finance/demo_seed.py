...
``import_service``, the ``categorize`` detectors, ``networth_service``) rather
...
DEMO_ACCOUNTS: tuple[DemoAccountSpec, ...] = (
    DemoAccountSpec("checking", "Chase Total Checking", "checking", "asset", 482_355),
    DemoAccountSpec("savings", "Ally Online Savings", "savings", "asset", 1_845_000),
    DemoAccountSpec("card", "Amex Blue Cash Preferred", "credit_card", "liability"),
    DemoAccountSpec("home", "Primary Residence", "property", "asset", 52_500_000),
    DemoAccountSpec("brokerage", "Fidelity Brokerage", "brokerage", "asset"),
    DemoAccountSpec("mortgage", "Mortgage", "loan", "liability", 31_842_000),
)
...
async def _write_transactions(
    ...
    *,
    ...
    Recent checking activity is handed to ``import_service`` as a QIF file so
...
async def seed_demo(
    ...
    *,
    ...
    net_worth_days = await networth_service.recompute_snapshots(
        db, owner_user_id=owner_user_id, start_date=window_start
    )
