...
def auto_discover_services() -> None:
    """
    Auto-discover services and components.

    This function automatically finds and imports all modules in the 'app/services'
    directory and component modules in 'app/components', allowing them to
    self-register their lifecycle tasks. This block runs once when called,
    triggering the execution of the module's top-level code.
    """
    # Discover user services
    services_dir = Path(__file__).parent.parent / "services"
    discover_and_import_services(services_dir)

    # Discover components (scheduler, etc.)
    components_dir = Path(__file__).parent.parent / "components"
    discover_and_import_components(components_dir)
...
def discover_and_import_components(components_path: str | Path) -> None:
    """
    Dynamically discovers and imports component modules.

    This function specifically looks for Python files (not directories) in the
    components directory and imports them. This allows components like scheduler.py
    to self-register with the lifecycle system.

    Args:
        components_path: The filesystem path to the components directory.
    """
    # Get the project's root directory to calculate the relative path
    project_root = Path.cwd()
    components_dir = Path(components_path)

    if not components_dir.exists():
        logger.debug(f"Components directory not found: {components_path}")
        return

    relative_path = components_dir.relative_to(project_root)
    package_name = ".".join(relative_path.parts)
    logger.info(f"--- Discovering components in '{package_name}' ---")

    # Look for Python files in the components directory
    for component_file in components_dir.glob("*.py"):
        if component_file.name.startswith("_") or component_file.name == "__init__.py":
            continue
        ...
        try:
            importlib.import_module(full_module_path)
            logger.info(f"Successfully imported component: {module_name}")
        except Exception as e:
            logger.error(f"Failed to import component: {module_name}, Error: {e}")
