...
def test_a_base_that_is_not_a_port_is_refused() -> None:
    with pytest.raises(ValueError):
        docs_port(70000)


def test_the_scan_stops_at_the_top_of_the_range(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """65536 is not a port. A base near the top must give up cleanly, not
    hand the socket a number it raises on."""
    monkeypatch.setattr(docs_port_module, "_is_free", lambda port: False)
    with pytest.raises(RuntimeError):
        docs_port(65530, max_attempts=20)
