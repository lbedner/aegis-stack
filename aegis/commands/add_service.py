...
def add_service_command(
    ...
    # Before anything is written, and for the same reason ``add``,
    # ``remove`` and ``remove-service`` do it: this command installs the
    # CURRENT template's files into a project generated by an older one,
    # and only the files the new service happens to touch. A 0.12.1 CLI
    # on a 0.11.0 project installed today's ``app/cli/migrate_gen.py``
    # beside that project's 0.11.0 ``alembic/env.py`` - which has nothing
    # to do with auth, so nothing updated it - and the generator's
    # options could not reach alembic. Every migration then failed, and
    # the project was left holding the service's code and none of its
    # schema (2026-09-20).
    validate_version_compatibility(target_path, command_name="add-service", force=force)
